...
template<int N, int D, typename Index, typename Value>
class Elevator {
 public:
  Elevator() {
    for (Line line = 0_line; line < line_size; ++line) {
      elevator[line].index = line;
      elevator[line].floor = 0_side;
    }
    ...
    elevator[0_line].left = &floor[0_side];
    elevator[0_line].right = &elevator[1_line];
    elevator[Line(line_size - 1)].left = &elevator[Line(line_size - 2)];
    elevator[Line(line_size - 1)].right = &floor[0_side];
    floor[0_side].left = &elevator[Line(line_size - 1)];
    floor[0_side].right = &elevator[Line(0)];
    for (Side side = 1_side; side <= N; ++side) {
  ...
  void inc(Line line) {
    Side current = Side{++elevator[line].floor};
